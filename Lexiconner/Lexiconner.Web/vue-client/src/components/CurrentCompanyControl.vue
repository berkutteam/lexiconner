        ...
        <div v-if="sharedState.userInfo.companies">
            ...
            <div class="row">
                <div class="col col-md-4">
                    <div class="d-flex justify-content-start align-items-start company-select-wrapper">
                        <multiselect 
                        ...
                        <button 
                            v-if="$store.getters.isUserHasPermissions(['CompanyUpdate'], $store.getters.currentCompanyId)"
                            v-on:click="onManageCompanyClick()" type="button" class="btn btn-outline-secondary ml-2">
                            <i class="fas fa-cog"></i>
                        </button>
                        <button 
                            v-if="$store.getters.isUserHasPermissions(['CompanyCreate'], null)"
                            v-on:click="onCreateCompanyClick($event)" type="button" class="btn btn-outline-success ml-2">
                            <i class="fas fa-plus"></i>
                        </button>
                        <loading-button 
                            v-if="$store.getters.currentCompanyId"
                            type="button"
                            v-bind:loading="sharedState.loading[privateState.storeTypes.COMPANY_LEAVE]"
    ...
    methods: {
        setSelectedCompany: function() {
            if(this.sharedState.userInfo) {
                let current = this.sharedState.userInfo.companies.find(x => x.isCurrent);
                if(current) {
        ...
        onCurrentCompanyChange: function(values) {
            ...
            this.$store.dispatch(storeTypes.USER_INFO_CURRENT_COMPANY_SET, {
        ...
        onManageCompanyClick: function() {
            this.$router.push({name: 'company-manage', params: {
