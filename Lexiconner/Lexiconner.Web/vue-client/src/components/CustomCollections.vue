<template>
  <div>
    <row-loader
      v-bind:visible="
        sharedState.loading[privateState.storeTypes.CUSTOM_COLLECTIONS_LOAD]
      "
      class="mb-2"
    ></row-loader>
    ...
    <div v-if="sharedState.customCollectionsResult">
        ...
        <div class="app-modal">
          <div class="app-modal-header">
            <div class="app-modal-title">
              <span v-if="privateState.modalMode === 'create'"
                >Create collection</span
              >
              <span v-if="privateState.modalMode === 'edit'"
                >Edit collection</span
              >
            </div>
            <div
              v-on:click="$modal.hide('custom-collection-edit')"
          ...
          <div class="app-modal-content">
            <form
              v-on:submit.prevent="
                createEditCustomCollection(privateState.modalMode)
              "
            >
              <div class="form-group">
                <label for="customCollectionModel__name">Name</label>
                <input
                  v-model="privateState.customCollectionModel.name"
                  type="text"
                  class="form-control"
                  id="customCollectionModel__name"
                  placeholder="Name"
                />
              </div>
              <loading-button
                ...
                v-bind:loading="
                  sharedState.loading[
                    privateState.storeTypes.CUSTOM_COLLECTION_CREATE
                  ] ||
                  sharedState.loading[
                    privateState.storeTypes.CUSTOM_COLLECTION_UPDATE
                  ]
                "
      ...
      >
        <div class="app-modal">
          <div class="app-modal-header">
            ...
            <div
              v-on:click="$modal.hide('custom-collection-delete')"
              class="app-modal-close"
            >
              <i class="fas fa-times"></i>
            </div>
          ...
          <div class="app-modal-content">
            <form
              v-on:submit.prevent="
                deleteCustomCollection(privateState.modalMode)
              "
            >
              <div class="form-check">
                <input
                  v-model="
                    privateState.customCollectionDeleteModel.isDeleteItems
                  "
                  class="form-check-input"
                  type="checkbox"
                  name="customCollectionDeleteModel_isDeleteItems"
                  id="customCollectionDeleteModel_isDeleteItems"
                />
                <label
                  class="form-check-label"
                  for="customCollectionDeleteModel_isDeleteItems"
                >
                  Move items to parent collection
                </label>
                <small class="form-text text-muted"
                  >If not checked the items in collection will be
                  deleted.</small
                >
              </div>
              <loading-button
                ...
                v-bind:loading="
                  sharedState.loading[
                    privateState.storeTypes.CUSTOM_COLLECTION_DELETE
                  ]
                "
      ...
      <!-- Word create/edit -->
      <word-create-update-modal ref="customCollections_wordCreateUpdateModal">
...
export default {
  ...
  data: function () {
    return {
      privateState: {
        ...
        customCollectionDeleteModel: _.cloneDeep(
          customCollectionDeleteModelDefault
        ),
  ...
  created: function () {
    ...
    this.$store
      .dispatch(storeTypes.CUSTOM_COLLECTIONS_LOAD, {})
      .then()
      .catch((err) => {
        console.error(err);
  ...
  methods: {
    ...
    onCreateFolder: function (parentFolder) {
      ...
      this.privateState.customCollectionModel.parentCollectionId =
        parentFolder.id;
      this.$modal.show("custom-collection-edit");
    },
    onUpdateFolder: function (sourceFolder) {
      this.privateState.modalMode = "edit";
      let collection = this.customCollectionsList.find(
        (x) => x.id === sourceFolder.id
      );
    ...
    onDuplicateFolder: function (sourceFolder) {
      this.$store
        .dispatch(storeTypes.CUSTOM_COLLECTION_DUPLICATE, {
    ...
    createCustomCollection: function () {
      this.$store
        .dispatch(storeTypes.CUSTOM_COLLECTION_CREATE, {
          data: {
            ...this.privateState.customCollectionModel,
          },
        })
        .then(() => {
          ...
          // reset
          this.privateState.customCollectionModel = _.cloneDeep(
    ...
    updateCustomCollection: function () {
      this.$store
        .dispatch(storeTypes.CUSTOM_COLLECTION_UPDATE, {
    ...
    markCustomCollectionAsSelected: function (customCollectionId) {
      this.$store
        .dispatch(storeTypes.CUSTOM_COLLECTION_MARK_AS_SELECTED, {
    ...
    markCustomCollectionAsSelectedDebounce: _.debounce(function (
      customCollectionId
    ) {
    ...
    deleteCustomCollection: function () {
      this.$store
        .dispatch(storeTypes.CUSTOM_COLLECTION_DELETE, {
          customCollectionId:
            this.privateState.customCollectionDeleteModel.customCollectionId,
          isDeleteItems:
            this.privateState.customCollectionDeleteModel.isDeleteItems,
        })
        .then(() => {
          this.$notify({
            group: "app",
          ...
          // reset
          this.privateState.customCollectionDeleteModel = _.cloneDeep(
            customCollectionDeleteModelDefault
          );
        })
        .catch((err) => {
