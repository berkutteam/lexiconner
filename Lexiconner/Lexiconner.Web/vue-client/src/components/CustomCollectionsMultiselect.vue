    ...
    <template slot="option" slot-scope="props">
      <div class="option__desc">
        <span class="option__title"
          >{{ props.option.levelPad }}{{ props.option.name }}</span
        >
      </div>
    </template>
  </multiselect>
...
export default {
  ...
  watch: {
    value: function (newValue, oldValue) {
      const isHasNewOptions =
        this.value.join("_") !==
        this.privateState.selectedCustomCollections.map((x) => x.id).join("_");
  ...
  methods: {
    ...
    setSelectedOptions: function (customCollectionIdsToSelect) {
      ...
      if (
        customCollectionIdsToSelect &&
        Array.isArray(customCollectionIdsToSelect)
      ) {
        let selectedOptions = customCollectionIdsToSelect.map(
          (customCollectionId) => {
            const option = this.options.find(
              (x) => x.id === customCollectionId
            );
            return {
              id: option.id,
              name: option.name,
