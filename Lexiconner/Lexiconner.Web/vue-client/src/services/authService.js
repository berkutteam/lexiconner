...
class AuthService {
  ...
  init(authConfig) {
    ...
    this.refreshTokensIntervalFunc =
      this.refreshTokensIntervalFunc ||
      setInterval(
        async function () {
          console.log(
            `AuthService: running refreshTokensInterval (every ${this.refreshTokensInterval} ms)...`
          );
          await this.refreshTokens({ withFullscreenLoader: false });
        }.bind(this),
        this.refreshTokensInterval
      );
  }
  ...
  async refreshTokens({ withFullscreenLoader = false } = {}) {
    ...
    if (isAuthenticated) {
      try {
        ...
        console.log(
          "AuthService:.refreshTokens: Silent token renewal successful. ",
          user
        );

        // manually update profile with the new claims
        // (profile isn't updated after token refresh)
        let nextClaims = await self.userInfoService.getClaims(
          user.access_token
        );
        ...
        console.error(
          "AuthService:.refreshTokens: Error from signinSilent:",
          err
        );
      ...
      let msg =
        "AuthService:.refreshTokens: Can't refresh token. User isn't authenticated.";
      console.error(msg);
