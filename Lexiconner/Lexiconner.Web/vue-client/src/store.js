...
export default new Vuex.Store({
  ...
  getters: {
    ...
    selectedLearningLanguageCode(state, getters) {
      ...
      return (
        (state.profile.learningLanguages.find((x) => x.isSelected) || {})
          .languageCode || null
      );
    },
  ...
  mutations: {
    ...
    [storeTypes.WORD_UPDATE_SET](state, payload) {
      let { data } = payload;
      if (state.wordsPaginationResult !== null) {
        state.wordsPaginationResult.items =
          state.wordsPaginationResult.items.map((x) => {
            if (x.id === data.id) {
              return { ...data };
            }
            return x;
    ...
    [storeTypes.WORD_DELETE_SET](state, payload) {
      ...
      if (state.wordsPaginationResult !== null) {
        state.wordsPaginationResult.items =
          state.wordsPaginationResult.items.filter((x) => x.id !== wordId);
      }
    },
    ...
    [storeTypes.WORD_PRONUNCIATION_AUDIO_SET](state, payload) {
      ...
      state.wordsPronunciationAudio[wordPronunciationAudio.word] =
    ...
    [storeTypes.USER_FILM_UPDATE_SET](state, payload) {
      ...
      if (state.userFilmsPaginationResult !== null) {
        state.userFilmsPaginationResult.items =
          state.userFilmsPaginationResult.items.map((x) => {
            if (x.id === data.id) {
              return { ...data };
            }
            return x;
    ...
    [storeTypes.USER_FILM_DELETE_SET](state, payload) {
      ...
      if (state.userFilmsPaginationResult !== null) {
        state.userFilmsPaginationResult.items =
          state.userFilmsPaginationResult.items.filter(
            (x) => x.id !== userFilmId
          );
      }
    },
  ...
  actions: {
    ...
    [storeTypes.WORD_PRONUNCIATION_AUDIO_LOAD](
      context,
      { languageCode, word }
    ) {
      let { commit, dispatch, state, getters } = context;
    ...
    [storeTypes.IMAGES_FIND_BY_LANGUAGE](
      context,
      { languageCode, search, limit }
    ) {
      let { commit, dispatch, getters } = context;
      commit(storeTypes.LOADING_SET, {
    ...
    [storeTypes.USER_DICTIONARY_WORD_SET_ADD](
      context,
      { wordSetId, selectedWordIds }
    ) {
      let { commit, dispatch, state, getters } = context;
          ...
          if (state.wordSetsPaginationResult !== null) {
            commit(storeTypes.WORD_SETS_LOAD_SET, {
              data: {
                ...
                items: [data, ...state.wordSetsPaginationResult.items],
              },
            });
          }
    ...
    [storeTypes.CUSTOM_COLLECTIONS_LOAD](context, params) {
      ...
      return api
        ...
        .then(({ data, ok }) => {
          ...
          const selectedCustomCollection =
            data.asList.find((x) => x.isSelected) || null;
    ...
    [storeTypes.CUSTOM_COLLECTION_UPDATE](
      context,
      { customCollectionId, data }
    ) {
      let { commit, dispatch, getters } = context;
      commit(storeTypes.LOADING_SET, {
    ...
    [storeTypes.CUSTOM_COLLECTION_MARK_AS_SELECTED](
      context,
      { customCollectionId }
    ) {
      let { commit, dispatch, getters } = context;
    ...
    [storeTypes.CUSTOM_COLLECTION_DUPLICATE](
      context,
      { customCollectionId, isDeleteItems }
    ) {
      let { commit, dispatch, getters } = context;
      commit(storeTypes.LOADING_SET, {
      ...
      return api
        .webApi()
        .duplicateCustomCollection({ customCollectionId, isDeleteItems })
        .then(({ data, ok }) => {
          commit(storeTypes.LOADING_SET, {
            target: storeTypes.CUSTOM_COLLECTION_DUPLICATE,
            loading: false,
