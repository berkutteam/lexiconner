...
class DateTimeUtil {
    ...
    get userTimeZoneId() {
        if (store.state.userInfo && store.state.userInfo.timeZone && store.state.userInfo.timeZone.timeZoneId) {
    ...
    formatToUserDisplayTime(datetime) {
        let format = ((store.userInfo || {}).formatSettings || {}).timeFormat || 'HH:mm';
        return this._getInUserTimeZoneOrUTC(datetime).format(format);
    }

    formatToUserDisplayDate(datetime) {
        let format = ((store.userInfo || {}).formatSettings || {}).dateFormat || 'YYYY-MM-DD';
        return this._getInUserTimeZoneOrUTC(datetime).format(format);
    }

    formatToUserDisplayDateTime(datetime) {
        let format = ((store.userInfo || {}).formatSettings || {}).dateTimeFormat || 'YYYY-MM-DD HH:mm';
        return this._getInUserTimeZoneOrUTC(datetime).format(format);
    }
    ...
    formatAccordingToInterval(datetime, from, to) {
        let duration = moment.duration(this._getInUserTimeZoneOrUTC(to).diff(this._getInUserTimeZoneOrUTC(from)));
