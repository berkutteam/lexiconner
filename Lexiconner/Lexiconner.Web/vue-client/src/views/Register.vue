            ...
            <div v-if="!globalLoading">
                ...
                <div v-if="privateState.currentStep === 'phone-enter'">
                    ...
                    <form v-on:submit.prevent="onSubmitPhoneEnterStep">
                        <div class="form-group">
                            ...
                            <small class="form-text text-muted">Phone number is used for confirmation.</small>
                        </div>
                        <loading-button 
                            type="submit"
                            v-bind:loading="sharedState.loading[privateState.storeTypes.AUTH_REGISTRATION_SMS_TOKEN_SEND]"
                ...
                <div v-if="privateState.currentStep === 'phone-validate'">
                    <form v-on:submit.prevent="onSubmitPhoneValidateStep">
                        <div class="form-group">
                            ...
                            <input v-model="privateState.model.phoneNumberConfirmationToken" type="text" class="form-control" id="inputConfiramtionCode" aria-describedby="confiramtionCodeHelp" placeholder="Confirmation code">
                            <small id="confiramtionCodeHelp" class="form-text text-muted">Code was sent to {{ privateState.model.phoneNumber }}.</small>
                        </div>
                        <button v-on:click="$event.preventDefault(); goToStep('phone-enter');" type="button" class="btn btn-outline-secondary">Back</button>
                        &nbsp;
                        <loading-button 
                            type="submit"
                            v-bind:loading="sharedState.loading[privateState.storeTypes.AUTH_REGISTRATION_SMS_TOKEN_VALIDATE]"
                            class="btn btn-outline-primary"
                        >Next</loading-button>
                    </form>
                </div>
                <div v-if="privateState.currentStep === 'registration-data-enter'">
                    <form v-on:submit.prevent="onSubmitRegistrationDataEnterStep">
                        <div class="form-group">
                            ...
                            <input v-model="privateState.model.email" v-bind:disabled="privateState.isEmailInputDisabled" type="email" class="form-control" id="inputEmail" aria-describedby="emailHelp" placeholder="Enter email" autocomplete="username">
                            <small id="emailHelp" class="form-text text-muted">Email is your username.</small>
                        </div>
                        <div class="form-group">
                            <label for="inputPassword">Password</label>
                            <input v-model="privateState.model.password" type="password" class="form-control" id="inputPassword" aria-describedby="passwordHelp" placeholder="Password" autocomplete="new-password">
                            <small v-if="registrationInfo && registrationInfo.passwordSuggestions" id="passwordHelp" class="form-text text-muted password-suggestion-list">
                                <p 
                        ...
                        <div class="form-group">
                            ...
                            <input v-model="privateState.model.passwordConfirm" type="password" class="form-control" id="inputPasswordConfirm" aria-describedby="passwordConfirmHelp" placeholder="Confirm password" autocomplete="new-password">
                        </div>
                        <div class="form-group">
                            <label for="inputName">Name</label>
                            <input v-model="privateState.model.name" type="text" class="form-control" id="inputName" aria-describedby="nameHelp" placeholder="Enter display name">
                            <small id="nameHelp" class="form-text text-muted">E.g. John Doe.</small>
                        </div>
                        <div class="form-group form-check">
                            <input v-model="privateState.model.isAcceptTermsOfUse" type="checkbox" class="form-check-input" id="inputAcceptTermsOfUse">
                            <label class="form-check-label" for="inputAcceptTermsOfUse">
                                I accept the 
                                <router-link v-bind:to="{ name: 'terms-of-use'}" target="_blank">Terms Of Use</router-link>
                            </label>
                        </div>
                        <button v-on:click="$event.preventDefault(); goToStep('phone-validate');" type="button" class="btn btn-outline-secondary">Back</button>
                        &nbsp;
                        <loading-button 
                            type="submit"
                            v-bind:loading="sharedState.loading[privateState.storeTypes.AUTH_REGISTER_AS_INTERNAL_USER]"
                ...
                <div v-if="privateState.currentStep === 'registration-completed'">
                    <div class="user-registered-wrapper">
                        ...
                        <div>
                            <router-link v-bind:to="{ name: 'login'}">Login</router-link> now and start working.
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    computed: {
        ...
        ...mapState({
            ...
            invitation: function(state) {
                if(this.query.invitationId && state.companyUserInvitation[this.query.invitationId]) {
                    return state.companyUserInvitation[this.query.invitationId];
                }
                return null;
            },
            globalLoading: function(state) {
                return state.loading[storeTypes.COMPANY_USER_INVITATION_VALIDATE] ||
    ...
    created: async function() {
        ...
        if(this.query.invitationId) {
            ...
            this.$store.dispatch(storeTypes.COMPANY_USER_INVITATION_VALIDATE, {
                ...
                if(data === true) {
                    ...
                    this.$store.dispatch(storeTypes.COMPANY_USER_INVITATION_LOAD, {
                        invitationId: this.query.invitationId,
                    }).then((data) => {
        ...
        if(this.query.preRegistrationUserId) {
            ...
            this.$store.dispatch(storeTypes.AUTH_PREREGISTRATION_USER_LOAD, {
                preRegistrationUserId: this.query.preRegistrationUserId,
            }).then((data) => {
                // preinput
                if(data) {
                    self.privateState.model.phoneNumber = (data.claims.find(x => x.type === 'phone_number') || {}).value || self.privateState.model.phoneNumber;
                    self.privateState.model.email = (data.claims.find(x => x.type === 'email') || {}).value || self.privateState.model.email;
                    self.privateState.model.name = (data.claims.find(x => x.type === 'name') || {}).value || self.privateState.model.name;
                } else {
        ...
        this.$store.dispatch(storeTypes.AUTH_REGISTRATION_INFO_LOAD, {}).then().catch(err => {
            console.error(err);
            notification.showErrorIfServerErrorResponseOrDefaultError(err);
        });
    },
    ...
    methods: {
        ...
        onSubmitPhoneEnterStep(e) {
            ...
            this.$store.dispatch(storeTypes.AUTH_REGISTRATION_SMS_TOKEN_SEND, {
        ...
        onSubmitPhoneValidateStep(e) {
            console.log('onSubmitPhoneValidateStep. model - ', this.privateState.model);
            this.$store.dispatch(storeTypes.AUTH_REGISTRATION_SMS_TOKEN_VALIDATE, {
                phoneNumber: this.privateState.model.phoneNumber,
        ...
        onSubmitRegistrationDataEnterStep(e) {
            console.log('onSubmitRegistrationDataEnterStep. model - ', this.privateState.model);
            let user = {
            ...
            this.$store.dispatch(storeTypes.AUTH_REGISTER_AS_INTERNAL_USER, {user}).then(async (data) => {
                // this.$notify({
                //     group: 'app',
                //     type: 'success',
        ...
        handleAfterRegistrationForExternalUser() {
            if(this.query.preRegistrationUserId) {
                ...
                authService.refreshTokens({withFullscreenLoader: true}).then(() => {
                    this.$router.push({
                        name: 'dashboard-home',
                    });
                    this.$store.commit(storeTypes.LOADING_SET, {
