<template>
  <div>
    <row-loader
      v-bind:visible="
        sharedState.loading[privateState.storeTypes.USER_DICTIONARY_LOAD]
      "
      class="mb-2"
    ></row-loader>
    ...
    <div class="app-card-nav mb-2">
      <div class="app-card-nav-item">
        <router-link
          v-bind:to="{ name: 'words-browse', params: {} }"
          class="app-card-nav-link"
        >
          <img
            class="app-card-nav-image app-card-nav-image--64x64"
            src="img/app-card-nav/icons8-browse-folder-2-64.png"
            alt=""
          />
      ...
      <div class="app-card-nav-item">
        <router-link
          ...
          v-on:click.native="onCreateWordClick"
          class="app-card-nav-link"
        >
          <img
            class="app-card-nav-image app-card-nav-image--64x64"
            src="img/app-card-nav/icons8-add-property-64.png"
            alt=""
          />
      ...
      <div class="app-card-nav-item">
        <router-link
          v-bind:to="{ name: '', params: {} }"
          v-on:click.native="onCreateWordSetClick"
          class="app-card-nav-link"
        >
          <img
            class="app-card-nav-image app-card-nav-image--64x64"
...
export default {
  ...
  methods: {
    ...
    onDeleteWordSetClick: function (wordSetId) {
      if (
        confirm(
          `You will lost words in word set and its' training status. Are you sure?`
        )
      ) {
        return this.$store
          .dispatch(storeTypes.USER_DICTIONARY_WORD_SET_DELETE, {
            wordSetId,
          })
          .then()
          .catch((err) => {
