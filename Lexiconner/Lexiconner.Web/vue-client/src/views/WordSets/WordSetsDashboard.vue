<template>
  <div class="dashboard-wrapper">
    <row-loader
      v-bind:visible="
        sharedState.loading[privateState.storeTypes.WORD_SETS_LOAD]
      "
      class="mb-2"
    ></row-loader>
    ...
    <div class="app-card-nav mb-2">
      <div class="app-card-nav-item">
        <router-link
          v-bind:to="{ name: 'wordset-create-create', params: {} }"
          class="app-card-nav-link"
        >
          <img
            class="app-card-nav-image app-card-nav-image--64x64"
            src="img/app-card-nav/icons8-add-folder-64.png"
            alt=""
          />
          <span class="app-card-nav-text">Create a new word set</span>
        </router-link>
      </div>
    </div>
          ...
          >
            ...
            >
              ...
              >
                ...
                <div
                  v-if="checkWordSetIsAlreadyAdded(item.id)"
                  class="card-already-added-message"
                >
    ...
    >
      <div class="app-modal">
        <div class="app-modal-header">
          ...
          <div
            v-on:click="$modal.hide('word-set-add-to-dictionary')"
          ...
          <form v-on:submit.prevent="addWordSetToDictionaryClick()">
            ...
            <div class="form-check">
              <input
                ...
                v-on:change="
                  (e) =>
                    onSelectAllWordsChange(
                      e,
                      privateState.addWordSetToDictionaryModel.wordSetId
                    )
                "
              />
            ...
            <div class="mb-2">
              ...
              >
                <input
                  ...
                  v-bind:checked="
                    privateState.addWordSetToDictionaryModel.selectedWordsIds.includes(
                      word.id
                    )
                  "
                />
            ...
            <loading-button
              ...
              v-bind:loading="
                sharedState.loading[
                  privateState.storeTypes.USER_DICTIONARY_WORD_SET_ADD
                ]
              "
...
export default {
  ...
  data: function () {
    return {
      privateState: {
        ...
        currentView:
          localStorage.getItem(`wordSetsBrowse_currentView`) || "cards", // ['cards']
  ...
  methods: {
    loadWordSets: function ({ offset = 0, limit = 50 } = {}) {
      return this.$store
        .dispatch(storeTypes.WORD_SETS_LOAD, {
          offset: offset,
          limit: limit,
        })
        .then()
    ...
    checkWordSetIsAlreadyAdded: function (wordSetId) {
      if (this.userDictionary) {
        return this.userDictionary.wordSets.some(
          (x) => x.sourceWordSetId === wordSetId
        );
      }
      return false;
    },
    getWordSetWords: function (wordSetId) {
      if (this.wordSets) {
        return (
          (this.wordSets.find((x) => x.id === wordSetId) || {}).words || []
        );
      }
      return [];
    },
    ...
    addWordSetToDictionaryClick: function () {
      return this.$store
        .dispatch(storeTypes.USER_DICTIONARY_WORD_SET_ADD, {
    ...
    onSelectWordChange: function (e, wordId) {
      const isAlreadySelected =
        this.privateState.addWordSetToDictionaryModel.selectedWordsIds.includes(
          wordId
        );
      if (isAlreadySelected) {
        this.privateState.addWordSetToDictionaryModel.selectedWordsIds =
          this.privateState.addWordSetToDictionaryModel.selectedWordsIds.filter(
            (x) => x !== wordId
          );
      } else {
        this.privateState.addWordSetToDictionaryModel.selectedWordsIds.push(
          wordId
        );
      }
    },
