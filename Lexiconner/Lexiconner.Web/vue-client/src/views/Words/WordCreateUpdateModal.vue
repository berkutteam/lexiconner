    ...
    >
      <div class="app-modal">
        <div class="app-modal-header">
          ...
          <div
            v-on:click="$modal.hide('word-create-edit')"
        ...
        <div class="app-modal-content">
          ...
          <form v-on:submit.prevent="createEditWord()">
            <div class="form-group">
              ...
              <input
                v-model="privateState.wordModel.word"
                type="text"
                class="form-control"
                id="wordModel__title"
                placeholder="Word"
            ...
            <div class="form-group">
              <span
                v-on:click="
                  onFavoriteChange(!privateState.wordModel.isFavourite)
                "
                class="cursor-pointer"
              >
                <i
                  v-if="privateState.wordModel.isFavourite"
            ...
            <loading-button
              ...
              v-bind:loading="
                ...
                sharedState.loading[privateState.storeTypes.WORD_UPDATE]
              "
    ...
    >
      <div class="app-modal">
        <div class="app-modal-header">
          ...
          <div
            v-on:click="$modal.hide('word-examples')"
            class="app-modal-close"
          >
            <i class="fas fa-times"></i>
          </div>
        ...
        <div class="app-modal-content">
          <row-loader
            v-bind:visible="
              sharedState.loading[privateState.storeTypes.WORD_EXAMPLES_LOAD]
            "
          ></row-loader>
            ...
            >
              <div class="form-check mb-2">
                <input
                  ...
                  v-on:change="
                    (e) => toggleSuggestedWordExample(exampleOption.randomId)
                  "
                  v-bind:checked="
                    privateState.selectedWordExampleIds.includes(
                      exampleOption.randomId
                    )
                  "
                ...
                />
                <label
                  for="`example-${exampleOption.randomId}`"
...
export default {
  ...
  computed: {
    ...
    canLoadWordExamples: function () {
      return (
        this.privateState.wordModel.wordLanguageCode &&
        this.privateState.wordModel.word
      );
    },
    ...
    wordExamplesOptions: function () {
      return !this.wordExamples
        ? []
        : this.wordExamples.examples.filter((x) => {
  ...
  methods: {
    ...
    loadWordExamples: function ({ languageCode, word } = {}) {
      return this.$store
        .dispatch(storeTypes.WORD_EXAMPLES_LOAD, {
          languageCode,
          word,
        })
        .then()
    ...
    toggleSuggestedWordExample: function (exampleRandomId) {
      const isAlreadySelected =
        this.privateState.selectedWordExampleIds.includes(exampleRandomId);
      if (isAlreadySelected) {
        this.privateState.selectedWordExampleIds =
          this.privateState.selectedWordExampleIds.filter(
            (x) => x !== exampleRandomId
          );
      } else {
    ...
    addWordExamples: function () {
      var selectedExamples = this.wordExamplesOptions.filter((x) =>
        this.privateState.selectedWordExampleIds.includes(x.randomId)
      );
    ...
    updateWord: function () {
      this.$store
        .dispatch(storeTypes.WORD_UPDATE, {
