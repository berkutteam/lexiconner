    ...
    <div class="row">
      <div class="col-12">
        <row-loader
          v-bind:visible="
            sharedState.loading[
              privateState.storeTypes.WORD_TRAINING_BUILDWORDS_START
            ]
          "
          class="mb-2"
        ></row-loader>
              ...
              >
                ...
                >
                  <button
                    ...
                    v-bind:class="{
                      'btn-outline-danger':
                        privateState.currentWordInvalidPartsIndexes.includes(
                          partIndex
                        ),
                    }"
                    v-bind:disabled="
                      privateState.currentWordEnteredPartsIndexes.includes(
                        partIndex
                      )
                    "
                    type="button"
                  >
                    <span v-if="wordPart && wordPart.trim()">{{
                      wordPart
                    }}</span>
...
export default {
  ...
  computed: {
    ...
    isAllTrained: function () {
      return (
        this.trainingBuildWords !== null &&
        this.trainingBuildWords.items.length === 0
      );
    },
  ...
  methods: {
    loadTraining: function () {
      return this.$store
        .dispatch(storeTypes.WORD_TRAINING_BUILDWORDS_START, {
    ...
    onWordPartEntered: function (enteredWordPart) {
      ...
      const enteredWordPartIndex = this.currentItem.wordParts
        ...
        .find(
          (x) =>
            x.part === enteredWordPart &&
            !this.privateState.currentWordEnteredPartsIndexes.includes(x.index)
        ).index;
      if (enteredWordPartIndex === -1) {
        return;
      }
      const nextPartIndex =
        this.privateState.currentWordEnteredPartsIndexes.length;
      const isCorrectPart =
        enteredWordPart === this.currentItem.correctWordParts[nextPartIndex];

      if (isCorrectPart) {
        this.privateState.currentWordInvalidPartsIndexes = [];
        this.privateState.currentWordEnteredParts.push(enteredWordPart);
        this.privateState.currentWordEnteredPartsIndexes.push(
          enteredWordPartIndex
        );
      } else {
        this.privateState.currentWordInvalidPartsIndexes.push(
          enteredWordPartIndex
        );
      }

      const isAllPartsEntered =
        this.privateState.currentWordEnteredParts.length ===
        this.currentItem.wordParts.length;

      if (isAllPartsEntered) {
        const resultAnswerWord =
          this.privateState.currentWordEnteredParts.join("");
    ...
    onShowClick: function () {
      ...
      this.privateState.currentWordInvalidPartsIndexes =
        this.currentItem.wordParts.map((x, i) => i);
    ...
    saveTraining: function () {
      ...
      return this.$store
        .dispatch(storeTypes.WORD_TRAINING_BUILDWORDS_SAVE, {
          data: {
