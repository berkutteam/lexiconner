    ...
    <div class="row">
      <div class="col-12">
        <row-loader
          v-bind:visible="
            sharedState.loading[
              privateState.storeTypes.WORD_TRAINING_LISTENWORDS_START
            ]
          "
          class="mb-2"
        ></row-loader>
        ...
        <div class="words-tarining-wrapper words-learn-listenwords-wrapper">
          ...
          <div
            v-if="!isAllTrained && currentItem"
            class="card bg-light training-card"
          >
            <!-- Image -->
            <img
              v-if="
                currentItem.word.images && currentItem.word.images.length !== 0
              "
              class="card-img-top training-image"
              v-bind:src="currentItem.word.images[0].url"
              v-bind:alt="currentItem.word.word"
            />
            <img
              v-else
              class="card-img-top training-image"
            ...
            <div class="card-body">
              ...
              <div
                class="
                  trainig-word-pronunciation-audio
                  d-flex
                  flex-row
              ...
              >
                ...
                <audio
                  v-bind:ref="`pronunciationAudioEl_${currentItem.word.id}`"
                  v-if="getCurrentWordPronunciationAudio()"
                  controls
                  class="hidden"
                >
                  <source
                    v-if="getCurrentWordPronunciationAudio().audioMp3Url"
                    v-bind:src="getCurrentWordPronunciationAudio().audioMp3Url"
                    type="audio/mpeg"
                  />
                  <source
                    v-if="getCurrentWordPronunciationAudio().audioOggUrl"
                    v-bind:src="getCurrentWordPronunciationAudio().audioOggUrl"
                    type="audio/ogg"
                  />
              ...
              >
                <input
                  ...
                  v-on:change="
                    (e) => onCurrentWordEnteredChange(e.target.value)
                  "
                  type="email"
                  ...
                  v-bind:class="{
                    'border-danger':
                      privateState.isCurrentItemAnsweredCorrectly === false,
                    'border-success':
                      privateState.isCurrentItemAnsweredCorrectly === true,
                  }"
              ...
              <div v-if="privateState.isShowCurrentItemDetails">
                <hr />
                <div
                  v-if="privateState.isShowCurrentItemDetails"
                  class="card-text mb-1 training-description"
                >
                  <div>{{ currentItem.word.word }}</div>
                </div>
                <div
                  v-if="privateState.isShowCurrentItemDetails"
                  class="card-text text-secondary training-example-text mb-1"
                >
                  <div
            ...
            <div class="card-bottom-controls">
              ...
              <div class="card-bottom-control-item dropdown">
                ...
                >
                  <button
                    v-on:click="onWordMarkAsLearnedClick(currentItem.word.id)"
                    class="dropdown-item"
                    type="button"
                  >
              ...
              <span
                ...
                class="card-bottom-control-item text-danger"
              >
                <i class="fas fa-question"></i>
              </span>
              <span
                v-if="!privateState.isCurrentItemAnswered"
                v-on:click="submitAnswer()"
                class="card-bottom-control-item text-success"
              >
              ...
              <span
                ...
                v-bind:class="{ disabled: !privateState.isCurrentItemAnswered }"
              >
          ...
          <div class="mt-2">
            <small>
              <div class="text-center">
                <span
                  >{{ privateState.currentItemIndex + 1 }} /
              ...
              <div>
                <span>
                  ...
                  <span class="badge badge-success">{{
                    privateState.summary.correctItemsCount
                  }}</span>
                </span>
                <span class="ml-1">
                  Incorrect:
                  <span class="badge badge-danger">{{
                    privateState.summary.incorrectItemsCount
                  }}</span>
                </span>
              </div>
              <div v-if="privateState.isTrainingFinished" class="mt-2">
                <div>
                  <router-link
                    v-bind:to="{ name: 'trainings-dashboard', params: {} }"
                    class="btn custom-btn-normal btn-sm text-white"
                  >
                    <i class="fas fa-chevron-left mr-1"></i>
                    <span>Back</span>
                  </router-link>
                  <button
                    v-on:click="startTraining"
                    type="button"
                    class="btn custom-btn-normal btn-sm ml-1"
                  >
                    Train next
...
export default {
  ...
  computed: {
    ...
    isAllTrained: function () {
      return (
        this.trainingListenWords !== null &&
        this.trainingListenWords.items.length === 0
      );
    },
  ...
  methods: {
    loadTraining: function () {
      return this.$store
        .dispatch(storeTypes.WORD_TRAINING_LISTENWORDS_START, {
    ...
    checkCurrentWordPronunciationAudioExists: function () {
      return (
        this.sharedState.wordsPronunciationAudio &&
    ...
    getCurrentWordPronunciationAudio: function () {
      ...
      let currentWordPronunciationAudio =
        this.sharedState.wordsPronunciationAudio &&
    ...
    onWordPronunciationAudioClick: function () {
      this.loadWordPronunciationAudio({
        ...
        if (
          this.getCurrentWordPronunciationAudio() != null &&
          this.$refs[refKey]
        ) {
          setTimeout(() => {
    ...
    submitAnswer: function () {
      const isCorrect =
        this.privateState.currentWordEntered.trim() ===
        this.currentItem.correctAnswer;
    ...
    handleItemResponse: function ({ wordId, isCorrect, answer }) {
      let isHandled = this.privateState.itemResults.some(
        (x) => x.wordId === wordId
      );
      if (isHandled) {
        return;
      ...
      this.privateState.summary = {
        correctItemsCount: isCorrect
          ? this.privateState.summary.correctItemsCount + 1
          : this.privateState.summary.correctItemsCount,
        incorrectItemsCount: !isCorrect
          ? this.privateState.summary.incorrectItemsCount + 1
          : this.privateState.summary.incorrectItemsCount,
      };
    },
    ...
    saveTraining: function () {
      ...
      return this.$store
        .dispatch(storeTypes.WORD_TRAINING_LISTENWORDS_SAVE, {
          data: {
            trainingType: this.trainingListenWords.trainingType,
            itemsResults: [...this.privateState.itemResults],
          },
        })
        .then(() => {})
        .catch((err) => {
    ...
    loadWordPronunciationAudio: function ({ languageCode, word } = {}) {
      return this.$store
        .dispatch(storeTypes.WORD_PRONUNCIATION_AUDIO_LOAD, {
          languageCode,
          word,
        })
        .then()
