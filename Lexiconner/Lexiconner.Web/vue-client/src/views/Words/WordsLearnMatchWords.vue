    ...
    <div class="row">
      <div class="col-12">
        <row-loader
          v-bind:visible="
            sharedState.loading[
              privateState.storeTypes.WORD_TRAINING_MATCHWORDS_START
            ]
          "
          class="mb-2"
        ></row-loader>
        ...
        <div class="words-learn-matchwords-wrapper">
          ...
          <div
            v-if="!isAllTrained && trainingMatchWords"
          ...
          >
            ...
            <div class="card-img-top">
              <div class="image-grid image-grid--5images">
                ...
                >
                  <img
                    v-if="item.word.images && item.word.images.length !== 0"
                    class="training-image"
                    v-bind:src="item.word.images[0].url"
                    v-bind:alt="item.word.word"
                  />
                    ...
                    <multiselect
                      ...
                      v-bind:class="{
                        ...
                        'multiselect--correctAnswer':
                          privateState.isShowAnswers &&
                          checkItemAnswerCorrect(item.word.id) === true,
                        'multiselect--wrongAnswer':
                          privateState.isShowAnswers &&
                          checkItemAnswerCorrect(item.word.id) === false,
                      }"
                      ...
                      v-on:input="
                        (possibleOption) =>
                          onOptionSelect(item.word.id, possibleOption)
                      "
                    >
                    </multiselect>
                  </div>
            ...
            <div
              v-if="!privateState.isTrainingFinished"
              class="card-bottom-controls"
            >
              <span
                v-on:click="onShowAnswersClick()"
                class="card-bottom-control-item text-danger"
              >
                <i class="fas fa-question"></i>
              </span>
              <span
                v-on:click="onSubmitAnswerClick()"
                class="card-bottom-control-item text-success"
                v-bind:class="{ disabled: !isCanBeSubmited }"
              >
...
export default {
  ...
  computed: {
    ...
    isAllTrained: function () {
      return (
        this.trainingMatchWords !== null &&
        this.trainingMatchWords.items.length === 0
      );
    },
    isCanBeSubmited: function () {
      // when all selects are choosen
      return (
        this.privateState.itemResults.length ===
        this.trainingMatchWords.items.length
      );
    },
    ...
    leftOptions: function () {
      if (this.trainingMatchWords) {
        ...
        return this.trainingMatchWords.possibleOptions.filter((x) => {
          return !this.privateState.wordSelectedOptions.some(
            (y) => y.randomId === x.randomId
          );
        });
      }
      return [];
  ...
  methods: {
    loadTraining: function () {
      return this.$store
        .dispatch(storeTypes.WORD_TRAINING_MATCHWORDS_START, {
    ...
    startTraining: function () {
      ...
      this.privateState.wordSelectedOptions = [];
    ...
    onOptionSelect: function (wordId, possibleOption) {
      this.handleItemResponse({
        itemId: wordId,
        isCorrect: possibleOption.correctForWordId === wordId,
    ...
    onShowAnswersClick: function () {
      ...
      const unansweredItems = this.trainingMatchWords.items.filter((x) => {
        return !this.privateState.itemResults.some(
          (y) => y.itemId === x.word.id
        );
      });
    ...
    checkItemAnswerCorrect: function (itemId) {
      const answer =
        this.privateState.itemResults.find((y) => y.itemId === itemId) || null;
    ...
    saveTraining: function () {
      ...
      return this.$store
        .dispatch(storeTypes.WORD_TRAINING_MATCHWORDS_SAVE, {
          data: {
