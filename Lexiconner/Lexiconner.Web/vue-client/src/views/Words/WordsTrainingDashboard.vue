<template>
  <div class="dashboard-wrapper">
    <row-loader
      v-bind:visible="
        sharedState.loading[privateState.storeTypes.WORD_TRAINING_STATS_LOAD]
      "
      class="mb-2"
    ></row-loader>
    ...
    <div v-if="isLearningLanguageCodeSelected === true">
      ...
      <div class="app-card-nav mb-2">
        <div class="app-card-nav-item">
          <router-link
            v-bind:to="{ name: 'words-learn-falshcards', params: {} }"
            class="app-card-nav-link"
          >
            <img
              class="app-card-nav-image app-card-nav-image--64x64"
              src="img/app-card-nav/icons8-red-card-80.png"
              alt=""
            />
            <span class="app-card-nav-text">Flash cards</span>
            <span class="app-card-nav-secondary-text">{{
              getAvailableForTrainingWordCount(
                privateState.traingTypes.FlashCards
              )
            }}</span>
          </router-link>
        </div>
        <div class="app-card-nav-item">
          <router-link
            v-bind:to="{ name: 'words-learn-wordmeaning', params: {} }"
            class="app-card-nav-link"
          >
            <img
              class="app-card-nav-image app-card-nav-image--64x64"
              src="img/app-card-nav/icons8-rich-text-converter-96.png"
              alt=""
            />
            <span class="app-card-nav-text">Word-Meaning</span>
            <span class="app-card-nav-secondary-text">{{
              getAvailableForTrainingWordCount(
                privateState.traingTypes.WordMeaning
              )
            }}</span>
          </router-link>
        </div>
        <div class="app-card-nav-item">
          <router-link
            v-bind:to="{ name: 'words-learn-meaningword', params: {} }"
            class="app-card-nav-link"
          >
            <img
              class="app-card-nav-image app-card-nav-image--64x64"
              src="img/app-card-nav/icons8-dictionary-64.png"
              alt=""
            />
            <span class="app-card-nav-text">Meaning-Word</span>
            <span class="app-card-nav-secondary-text">{{
              getAvailableForTrainingWordCount(
                privateState.traingTypes.MeaningWord
              )
            }}</span>
          </router-link>
        </div>
        <div class="app-card-nav-item">
          <router-link
            v-bind:to="{ name: 'words-learn-matchwords', params: {} }"
            class="app-card-nav-link"
          >
            <img
              class="app-card-nav-image app-card-nav-image--64x64"
              src="img/app-card-nav/icons8-compare-64.png"
              alt=""
            />
            <span class="app-card-nav-text">Match words</span>
            <span class="app-card-nav-secondary-text">{{
              getAvailableForTrainingWordCount(
                privateState.traingTypes.MatchWords
              )
            }}</span>
          </router-link>
        </div>
        <div class="app-card-nav-item">
          <router-link
            v-bind:to="{ name: 'words-learn-buildwords', params: {} }"
            class="app-card-nav-link"
          >
            <img
              class="app-card-nav-image app-card-nav-image--64x64"
              src="img/app-card-nav/icons8-brick-wall-64.png"
              alt=""
            />
            <span class="app-card-nav-text">Build word</span>
            <span class="app-card-nav-secondary-text">{{
              getAvailableForTrainingWordCount(
                privateState.traingTypes.BuildWords
              )
            }}</span>
          </router-link>
        </div>
        <div class="app-card-nav-item">
          <router-link
            v-bind:to="{ name: 'words-learn-listenwords', params: {} }"
            class="app-card-nav-link"
          >
            <img
              class="app-card-nav-image app-card-nav-image--64x64"
              src="img/app-card-nav/icons8-foreign-language-sound-64.png"
              alt=""
            />
            <span class="app-card-nav-text">Listen word</span>
            <span class="app-card-nav-secondary-text">{{
              getAvailableForTrainingWordCount(
                privateState.traingTypes.ListenWords
              )
            }}</span>
          </router-link>
        </div>
              ...
              <div class="mb-3">
                <div class="h5">
                  ...
                  <span class="badge badge-secondary ml-1">{{
                    trainingStats.totalItemCount
                  }}</span>
                </div>
                <div class="h5">
                  <span>Trained</span>
                  <span class="badge badge-success ml-1">{{
                    trainingStats.trainedItemCount
                  }}</span>
                </div>
                <div class="h5">
                  <span>On training</span>
                  <span class="badge badge-info ml-1">{{
                    trainingStats.onTrainingItemCount
                  }}</span>
                </div>
              </div>
              <div>
                  ...
                  >
                    ...
                    <ul>
                      <li>
                        <span>Trained</span>
                        <span class="badge badge-success ml-1">{{
                          item.trainedItemCount
                        }}</span>
                      </li>
                      <li>
                        <span>Training</span>
                        <span class="badge badge-info ml-1">{{
                          item.onTrainingItemCount
                        }}</span>
                      </li>
                    </ul>
                  </li>
...
export default {
  ...
  methods: {
    loadTrainingsStats: function () {
      return this.$store
        .dispatch(storeTypes.WORD_TRAINING_STATS_LOAD, {})
        .then()
        .catch((err) => {
          console.error(err);
    ...
    getAvailableForTrainingWordCount: function (trainingType) {
      ...
      return (
        (
          this.trainingStats.trainingStats.find(
            (x) => x.trainingType === trainingType
          ) || {}
        ).availableForTrainingCount || 0
      );
    },
  },
};
