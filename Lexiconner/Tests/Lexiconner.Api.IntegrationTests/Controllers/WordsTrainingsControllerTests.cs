...
{
    public class WordsTrainingsControllerTests : TestBase
    {
        public WordsTrainingsControllerTests(CustomWebApplicationFactory<Startup> factory) : base(factory)
        {
        }
        ...
        public async Task GetTrainingItemsForFlashCards()
        {
            ...
            int count = 15;
            int limit = count;
            var wordsEntities = await _dataUtil.CreateWordsAsync(userEntity.Id, count);
        ...
        private Task GetAndSaveTrainings(out string accessToken)
        {
            ...
            int count = 15;
            int limit = count;
            var wordsEntities = _dataUtil.CreateWordsAsync(userEntity.Id, count).GetAwaiter().GetResult();
            ...
            wordsEntities = _dataUtil.GetWordsAsync(userEntity.Id).GetAwaiter().GetResult();
